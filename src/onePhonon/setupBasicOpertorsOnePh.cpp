    ...
    for (ulong ph1Ind = 0ul; ph1Ind < dimPhonon; ++ph1Ind) {
      for (ulong eInd1 = 0ul; eInd1 < 4ul; ++eInd1) {
        for (ulong eInd2 = 0ul; eInd2 < 4ul; ++eInd2) {
          HElectronic[toGlobalMatrixIndexOne(ptInd, ptInd, ph1Ind, ph1Ind, eInd1, eInd2)] = HElectronicSmall[eInd1 * 4ul + eInd2];
        }
      }
    }
  }
...
void setupOccSiteIOnePh(std::vector<std::complex<double>> &OccSiteI, const ulong site) {
  ...
  for (ulong ptInd = 0ul; ptInd < dimPhoton; ++ptInd) {
    for (ulong phInd = 0ul; phInd < dimPhonon; ++phInd) {
      for (ulong eInd1 = 0ul; eInd1 < 4ul; ++eInd1) {
        for (ulong eInd2 = 0ul; eInd2 < 4ul; ++eInd2) {
          OccSiteI[toGlobalMatrixIndexOne(ptInd, ptInd, phInd, phInd, eInd1, eInd2)] = occSmall[eInd1 * 4ul + eInd2];
        }
      }
    }
  }
