...
void calcTimeStep(const double tPoint,
  ...
  addMatricies(H, 1., Xpt, pumpPrefac * std::sin(wDrive * (tPoint - (.5 - std::sqrt(3.) / 6.) * dt)), H1);
  addMatricies(H, 1., Xpt, pumpPrefac * std::sin(wDrive * (tPoint - (.5 + std::sqrt(3.) / 6.) * dt)), H2);
  ...
  for (ulong ind1 = 0ul; ind1 < dimHOnePh; ++ind1) {
    for (ulong ind2 = 0ul; ind2 < dimHOnePh; ++ind2) {
      if (ind1 == ind2) {
        H1[ind1 * dimHOnePh + ind2] = std::exp(- II * eValsU[ind1] * dt);
        H2[ind1 * dimHOnePh + ind2] = std::exp(- II * eValsV[ind1] * dt);
      } else {
